...
export default function AccuracyBars({ predictions }: { predictions: Prediction[] }) {
  const tally = emptyRow();
  for (const p of predictions) {
    for (const ev of p.evaluations ?? []) {
      const cell = tally[ev.horizon];
      if (!cell) continue;
  ...
  const anyDecided = HORIZONS.some((h) => {
    const c = tally[h];
  ...
  if (!anyDecided) {
    return (
      <div className="chart-empty" style={{ height: 150 }}>
        Hit-rate appears once signals are scored (after 24h / 7d).
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {HORIZONS.map((h) => {
        const t = tally[h] ?? { correct: 0, incorrect: 0 };
